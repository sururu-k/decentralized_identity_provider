...
> ⚠️ 教育・議論用。自前実装の暗号を含み監査も受けていないので、本番で使ってはいけない。
...
ただし構造上の帰結が1つある。**ユーザーのデバイスが t 台のサーバと直接やり取りする必要がある**ため、
OAuth ゲートウェイを単なるプロキシにはできない（論点E に効く）。
...
**署名シェア `z_i = d_i + ρ_i·e_i + λ_i·s_i·c` は全てローカルの線形演算**なので、
