...
import { Shield, Key, Lock, CheckCircle2, RefreshCw, Send, ArrowRight, Server, Copy } from 'lucide-react';
...
export default function App() {
  ...
  const startSignOn = async () => {
    ...
    const mockJkt = '6cPQNImfbhqmjH65XD9U826uZYL5KR5Sm9bWm3NuQXM';
    addLog(`DPoP公開鍵サムプリント算出完了: cnf.jkt = ${mockJkt}`);
    ...
    addLog('各ノードがFROST署名シェア z_i を生成し、h_i で暗号化中 (ChaCha20-Poly1305 AAD付き)...');
    await new Promise((r) => setTimeout(r, 700));

    // Try live gateway backend API
    let token = '';
    try {
      ...
      if (data.success && data.id_token) {
        token = data.id_token;
        addLog('実動ゲートウェイと接続成功: トークンを受信・検証しました！');
      } else {
        throw new Error(data.error || 'Gateway returned non-success');
      }
    } catch {
      // Fallback in-browser deterministic simulation
      addLog('端末内高精度暗号シミュレーションを実行中...');
      const header = { alg: 'EdDSA', typ: 'JWT', kid: 'pasta-group-key-1' };
      const now = Math.floor(Date.now() / 1000);
      const payload = {
        iss: 'http://localhost:3000',
        sub: username === 'alice' ? 'usr_alice_12345' : 'usr_bob_67890',
        aud: 'zk_app_portal',
        iat: now,
        exp: now + 300,
      ...
      const b64u = (obj: any) =>
        btoa(JSON.stringify(obj))
          ...
          .replace(/=+$/, '');
      const sigMock =
        'v12dD_Z3lA7sF8kG_Ed25519ThresholdSignatureFromFROSTQuorumCombinedByLagrangeInterpolation_';
      token = `${b64u(header)}.${b64u(payload)}.${sigMock.substring(0, 43)}`;
    }

    // 4. Client Aggregation
    ...
    try {
      ...
      const parseB64 = (s: string) => JSON.parse(atob(s.replace(/-/g, '+').replace(/_/g, '/')));
      setDecodedToken({
        header: parseB64(parts[0]),
        payload: parseB64(parts[1]),
        signature: parts[2],
      });
    } catch {
