...
| `src/sdk/client.ts` | `client-sdk/client.ts` | ①`proxy` (インプロセス) 分岐と `PastaOAuthProxy` import を削除。あわせて `proxyUrl` を **必須の `string`** にした (移植元は真偽値判定だったので `""` = 同一オリジンを表現できなかった)。②`crypto.randomBytes(16)` → `new Uint8Array(16)` + `globalThis.crypto.getRandomValues`。③`JSON.parse(Buffer.from(bytes).toString("utf8"))` → `JSON.parse(new TextDecoder().decode(bytes))` (2 箇所)。④`onEvent` 進行コールバックを追加 (第 10 節のログ用、1 呼び出し = 1 行)。⑤`import "./buffer-shim.js"` を先頭に追加。ブラインド、header/payload オブジェクトとそのキー順、`createSigningInput`、`deriveAeadNonce` の引数、`aeadDecrypt` の AAD、集約順序はすべて移植元と同一 |
...
OAuth 連携パラメータ (`client_id`, `nonce`, `redirect_uri`, `state`) はクエリから読み、
...
4. **完了** — JWT の表示とコピー、**DPoP リフレッシュ**、RP への form_post 送信。
...
誤パスワードでの復号失敗、を確認する。検証に SDK 自身の `verifyJwt` を使わないのは、
...
3. **DPoP による送信者束縛 (RFC 9449)** — 端末内で作った使い捨て Ed25519 鍵に `cnf.jkt` で束縛し、
   リフレッシュはその鍵の proof を要求する。
