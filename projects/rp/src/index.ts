...
import { createRpServer } from "./server.js";
...
server.listen(port, () => {
  ...
  console.log(`[RP] IdP issuer:   ${config.issuer}`);
  console.log(`[RP] JWKS source:  ${config.idpInternalUrl}/jwks.json`);
  console.log(`[RP] client_id:    ${config.clientId}`);
  console.log(`[RP] redirect_uri: ${config.rpBaseUrl}/callback`);
