...
import { refreshResultFromWire, signOnResultFromWire } from "../gateway/wire.js";
 ...
 *
 * Ported from the monolith's `src/client-sdk/client.ts`. One change: the `proxyUrl`
 * (HTTP) branches decode the wire form of the proxy result instead of using the parsed
 * JSON directly. The monolith only ever exercised the in-process `proxy` branch, so its
 * HTTP branch handed raw JSON objects to code that needs `Uint8Array` points and could
 * never have aggregated a signature.
 */
