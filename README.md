...
発表資料を作る　
それぞれのサーバを分ける：docker compones
demoの動画もしくは写真を取る　: それぞれステップごとでコマ送り的な感じにして今何の暗号化技術を使っていて　どのような処理を行っているのか的な

要検討　
token endpointが従来のOAuthを壊していないかを確認する
リフレッシュの部分を従来のOAuthを壊していないか？を検証する
authorization codeの部分がOAuthに沿っているかを検証